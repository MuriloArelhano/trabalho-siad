...
try:
    ...
    cursor = conn.cursor()
    db_dataframe = pd.read_sql_query("""select data.mes, data.ano, cidade.nome as cidade, cidade.estado, dengue.n_casos, clima.temp_max_celsius, clima.temp_med_celsius, clima.temp_min_celsius, clima.umidade_relativa, clima.precipitacao_mensal_mm
        from dengue
...
final_dataframe['pop_infectada_percent'] = (db_dataframe['n_casos']*100)/db_dataframe['n_casos']

# %%
db_dataframe_graph = db_dataframe.pivot(
    index='data', columns='cidade', values='n_casos')
db_dataframe_graph.plot(title='Numero mensal de casos por cidade ',figsize=(15, 7), xlim=[
...
final_dataframe_graph.plot(figsize=(15, 7), title="Relação crescimento dos casos com precipitação (Rio de Janeiro)", xlim=[
    datetime.date(2012, 1, 1), datetime.date(2016, 2, 1)], style='-o')
...
final_dataframe_graph.plot(figsize=(15, 7), title="Relação crescimento dos casos temperatura média (Rio de Janeiro)", xlim=[
